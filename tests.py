...
class Test_Connection(unittest.TestCase):
    ...
    def test_connection_creation(self):
        """
        Test that the connection constructor saves the field as expected
        """
...
class TestDevice(unittest.TestCase):
    ...
    def test_is_host(self):
        ...
        self.assertEqual(Device._is_host(
    ...
    def test_is_switch(self):
        ...
        self.assertEqual(Device._is_switch(
            'Switch	41 "S-b8599f0300fc6de4"		# "MF0;r-ufm-sw95:MQM8700/U1" enhanced port 0 lid 13 lmc 0'),
            True)
...
class TestInfinibandTopologyParser(unittest.TestCase):
    """
    ...
    def test_printing(self):
        for test_file_path, origin_file_path, output_file in [
            [".\\testsFiles\small_topo_file_output.txt", ".\TopologyFiles\small_topo_file",
             'small_topo_file_output.txt'],
            [".\\testsFiles\large_topo_file_output.txt", ".\TopologyFiles\large_topo_file",
             'large_topo_file_output.txt']]:
            parser = InfinibandTopologyParser(origin_file_path)
            parser.parse()
            parser.print_devices_connections()
            test_chunks = set(''.join(chunk) for chunk in InfinibandTopologyParser.file_chunk_generator(test_file_path))
            output_chunks = set(''.join(chunk) for chunk in InfinibandTopologyParser.file_chunk_generator(output_file))
            self.assertEqual(test_chunks, output_chunks)
